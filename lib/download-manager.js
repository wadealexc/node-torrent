...
class DownloadManager extends EventEmitter {
    ...
    constructor (downloadInfo, peerId, infoHash) {
        ...
        // Number of peers currently assigned to tasks
        this.currentlyAssigned = 0;
        // Number of total peers connected for download
    ...
    startDownload(peers) {
        ...
        peers.forEach(peer => {
            ...
            p.on('ready', () => {
                this.totalConnected++;
                debug('%d ready for work (%d on jobs)', this.totalConnected, this.currentlyAssigned);

                // Now that this peer is connected, we want to know if the connection is closed
                p.once('close', () => {
                    this.totalConnected--;
                    debug('%d total connected (%d on jobs)', this.totalConnected, this.currentlyAssigned);
                });
            ...
            p.on('download complete', (piece) => {
                ...
                if (hash !== this.pieceHashes[piece.index]) {
                    ...
                    debug('#%d hash is valid!', piece.index);
                    this.results.push(piece.index);
                    debug('%d of %d pieces received', this.results.length, this.pieceHashes.length);

                    if (this.results.length === this.pieceHashes.length) {
                        debug('FILE COMPLETELY DOWNLOADED AND VERIFIED!');
                        process.exit(1);
                    }
                }
    ...
    _tryAssignWork() {
        ...
        debug('assigned %d of %d available peers (jobs left: %d)', peersAssigned.length, this.peerQueue.size(), this.workQueue.size());
        if (peersAssigned.length === 0) {
            debug('unable to assign any of: %O', this.workQueue.queue);
        }
